...
data_analyte <- read.csv2(file.path(paste0(path, '/data'), file_names[1]),
...
pdf("results/albumin_tree.pdf")
par(rpart.plot(sex_age_rpart, box.palette = "RdBu", main = paste0(var_analyte, " ", var_unit)))
dev.off()
```
# Interpretation rpart:
- 2 splits at 350 and 3706 days. No sex-dependence
...
tbl %>% write_csv('results/albumin_tbl.csv')
```

```{r comparison using Harris & Boyd}
...
calc_HarrisBoyd(rpart_stats_unisex[2,], caliper_stats_unisex[3,])
...
calc_HarrisBoyd(rpart_stats_unisex[3,], caliper_stats_male[1,])
calc_HarrisBoyd(rpart_stats_unisex[3,], caliper_stats_female[1,])

## und anschließend interpretieren
```
## Zusammenfassung Albumin
- Vermutlich deutlich andere Interpretation mittels Harris&Boyd durch anders berechnete RI intervalle mittels reflim. Harris&Boyd empfehlen split, wenn z > critical value ist.
- kein Unterschied zwischen den ersten beiden CALIPER RI -> Begründung?
- rpart split sind relevant
- im Vergleich rpart und caliper keine relevanten Unterschiede
-  caliper male vs female relevant
- rpart unisex vs caliper male/female nicht relevant
- Bestätigung, dass rpart gut separiert hat, bzw besser ist als caliper
...
print(gr)
ggsave(file="results/alb_fig_c.pdf", plot=gr)
...
print(gc)
ggsave(file="results/alb_fig_b.pdf", plot=gc)
```
